...
// //   return (
// //     <div
// //       className={`fixed left-1/2 -translate-x-1/2 bottom-40 md:bottom-24 z-36 transition-all duration-500 transform ${
// //         isVisible ? "translate-x-0 opacity-100" : "translate-x-full opacity-0"
// //       }`}
// //     >
// //       <div className="mx-auto w-[90vw] md:w-96 lg:w-[480px] max-w-[480px] bg-card border-l-4 border-primary rounded-lg shadow-2xl overflow-hidden">
// //         {/* Header */}
// //         <div className="bg-primary/10 px-8 py-6 flex items-start justify-between">
// //           <div className="flex items-start gap-3 flex-1">
// //             <AlertCircle className="w-6 h-6 text-primary flex-shrink-0 mt-0.5" />
...
const announcements: Announcement[] = [
  {
    ...
    title: "Our Next Big Event",
    description:
      "Join us on March 15th for a charity fundraiser to support our maternal health programs.",
    color: "primary",
  },
  {
    id: "donate",
    title: "Donate to Save Lives",
    description:
      "Your donation of just $5 can provide basic healthcare to a child in need.",
    color: "primary",
  },
  {
    id: "updates",
    title: "Sign Up for Updates",
    description:
      "Get the latest news about our programs and impact stories delivered to your inbox.",
    color: "primary",
  },
]
...
export default function SlidingAnnouncementPopup() {
  ...
  return (
    <div
      className={`fixed right-2 md:right-0 bottom-16 md:bottom-24 z-36
    ...
    >
      <div
        className="
          w-[82vw] max-w-xs
          md:w-[480px] md:max-w-none
          bg-card border-l-4 border-primary
          rounded-lg shadow-xl overflow-hidden
        "
      >
        {/* Header */}
        <div className="bg-primary/10 px-4 py-4 md:px-8 md:py-6 flex items-start justify-between">
          <div className="flex items-start gap-3 flex-1">
            <AlertCircle className="w-5 h-5 md:w-6 md:h-6 text-primary mt-0.5 flex-shrink-0" />
            <div>
              <h3 className="font-semibold text-base md:text-lg text-foreground">
                {current.title}
              </h3>
              <p className="text-xs md:text-sm text-muted-foreground mt-1 md:mt-2 leading-relaxed">
                {current.description}
              </p>
            </div>
          </div>
        ...
        {/* Indicator Dots */}
        <div className="flex gap-2 px-4 md:px-8 py-2 justify-center bg-muted/50">
          {announcements.map((_, idx) => (
            <button
              key={idx}
              onClick={() => setCurrentIndex(idx)}
        ...
        {/* Actions */}
        <div className="px-4 md:px-8 py-3 md:py-4 bg-primary/5 flex gap-2">
          <button className="flex-1 px-3 py-1.5 md:px-4 md:py-2 bg-primary text-primary-foreground rounded-md font-semibold text-xs md:text-sm hover:bg-primary/90 transition-colors">
            {current.id === "event" && "Learn More"}
            {current.id === "donate" && "Donate Now"}
            {current.id === "updates" && "Sign Up"}
          </button>
          ...
          <button
            onClick={() => setDismissed(true)}
            className="px-3 py-1.5 md:px-4 md:py-2 bg-muted text-foreground rounded-md font-semibold text-xs md:text-sm hover:bg-muted/80 transition-colors"
          >
            Dismiss
          </button>
        </div>
